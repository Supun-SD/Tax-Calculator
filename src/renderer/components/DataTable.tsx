...
export function DataTable<T extends { id?: number | string }>({
    ...
    const searchableKeys = searchKeys || columns
    ...
    return (
        <div className={`bg-surface rounded-[30px] p-3 ${className}`}>
            {/* Table */}
            <table className="w-full table-fixed">
                <thead className="bg-surface-2 rounded-t-[30px]">
                    <tr>
                        {columns.map((column, index) => (
                            <th
                                key={String(column.key)}
                                className={`text-left py-4 px-8 font-bold text-gray-300 ${column.sortable !== false ? 'cursor-pointer hover:text-white' : ''
                                    } ${index === 0 ? 'rounded-tl-[20px]' : ''
                                    } ${index === columns.length - 1 && !showActions ? 'rounded-tr-[20px]' : ''
                                    } ${column.width || ''}`}
                        ...
                        {showActions && (
                            <th className="text-right py-4 px-8 font-bold text-gray-300 rounded-tr-[20px] w-32">
                                Actions
                            </th>
                        )}
                    </tr>
                ...
                <tbody>
                    {currentData.length > 0 ? (
                        currentData.map((row, rowIndex) => (
                            <tr
                                key={row.id || rowIndex}
                                className={`border-b border-[#484848] hover:bg-gray-800 transition-colors ${onRowClick ? 'cursor-pointer' : ''
                                    }`}
                            ...
                            >
                                {columns.map((column) => (
                                    <td
                                        key={String(column.key)}
                                        className="py-4 px-8 text-white text-sm"
                                    >
                        ...
                        <tr>
                            <td colSpan={columns.length + (showActions ? 1 : 0)} className="text-ph text-center text-lg p-16">
                                No records
                            </td>
                        </tr>
                    )}
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <select
                                ...
                                className="bg-surface-2 text-white text-sm px-2 py-1 rounded border border-gray-600 focus:outline-none focus:border-blue-500"
                            >
