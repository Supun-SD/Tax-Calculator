...
interface CalculationContextType {
  ...
  setCurrentCalculation: (
    calculation: Calculation | CalculationReq | null
  ) => void;
...
export const CalculationProvider: React.FC<CalculationProviderProps> = ({
  ...
  const [currentCalculation, setCurrentCalculation] = useState<
    Calculation | CalculationReq | null
  >(null);
  ...
  const calculateAndUpdateGrossIncomeTax = useCallback(
    (calculation: Calculation | CalculationReq) => {
      ...
      const foreignIncome =
        calculation.calculationData.grossIncomeTax?.foreignIncome.total ?? 0;
      const foreignIncomeTaxRate =
        calculation.calculationData.settings.reliefsAndAit.foreignIncomeTaxRate;
  ...
  const calculateAndUpdateTotalPayableTax = useCallback(
    (calculation: Calculation | CalculationReq) => {
      const grossIncomeTax =
        calculation.calculationData.grossIncomeTax?.total ?? 0;
      const totalAitInterest =
        calculation.calculationData.sourceOfIncome.interestIncome?.totalAit ??
        0;
      const totalAitRent =
        calculation.calculationData.sourceOfIncome.rentalIncome.totalAit ?? 0;
      const totalApit =
        calculation.calculationData.sourceOfIncome.employmentIncome
          ?.apitTotal ?? 0;
      const whtProfessionalFee =
        calculation.calculationData.sourceOfIncome.businessIncome?.whtTotal ??
        0;
  ...
  const calculateAndUpdateBalancePayableTax = useCallback(
    (calculation: Calculation | CalculationReq) => {
      const totalPayableTax = calculation.calculationData.totalPayableTax;
      const quarterlyPayments = calculation.calculationData.balancePayableTax
        ?.quarterly ?? {
        one: 0,
        two: 0,
        three: 0,
      ...
      const totalQuarterlyPayments =
        quarterlyPayments.one +
        quarterlyPayments.two +
        quarterlyPayments.three +
        quarterlyPayments.four;
  ...
  const createNewCalculation = useCallback(
    async (isEditing: boolean, calculationId?: number) => {
      setIsLoading(true);
      try {
  ...
  const updateCalculationAccount = useCallback(
    (accountId: number, year: string) => {
  ...
  const updateEmploymentIncome = useCallback(
    (employmentIncome: EmploymentIncome | null) => {
      if (currentCalculation) {
  ...
  const updateRentalIncome = useCallback(
    (rentalIncome: RentalIncome | null) => {
      if (currentCalculation) {
        ...
        if (rentalIncome && rentalIncome.applyRentRelief !== false) {
          const rentReliefAmount =
            (rentalIncome.total *
  ...
  const updateInterestIncome = useCallback(
    (interestIncome: InterestIncome | null) => {
      if (currentCalculation) {
        const sourceOfIncome =
          currentCalculation.calculationData.sourceOfIncome;
  ...
  const updateDividendIncome = useCallback(
    (dividendIncome: DividendIncome | null) => {
      if (currentCalculation) {
        const sourceOfIncome =
          currentCalculation.calculationData.sourceOfIncome;
  ...
  const updateBusinessIncome = useCallback(
    (businessIncome: BusinessIncome | null) => {
      if (currentCalculation) {
        const sourceOfIncome =
          currentCalculation.calculationData.sourceOfIncome;
  ...
  const updateSolarRelief = useCallback(
    (solarRelief: number) => {
      if (currentCalculation) {
        const updatedCalculation = {
          ...currentCalculation,
