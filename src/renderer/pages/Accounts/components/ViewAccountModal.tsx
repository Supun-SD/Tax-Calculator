...
import { RiAccountCircleFill, RiCalculatorLine, RiEyeLine, RiEditLine, RiDownloadLine } from "react-icons/ri";
...
const ViewAccountModal: React.FC<ViewAccountModalProps> = ({
    ...
    const accountCalculations = calculations.filter((calc: Calculation) => calc.account.id === account?.id);

    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title={<Flex align="center" gap="2" mt="4">
                <RiAccountCircleFill size={72} color='gray' />
                <div>
                    <Text as="div" size="7" weight="bold">{account?.name}</Text>
                    <Text as="div" size="3" mb="1" weight="medium" color='gray'>{account?.tinNumber}</Text>
                </div>
            </Flex>}
        >
            <div>
                <Text size="5" weight="bold">Calculations</Text>
